...
class Camera(object):
  ...
  def get_img_raw(self, timeout, size = (320, 240)):
    ...
    if frame.size != (2*self.height*self.width):
      return None
    data = np.frombuffer(frame.data, dtype=np.uint16).reshape(self.height, self.width)
...
class FootGui(object):
  ...
  _right_frame =  [      
                    [sg.T('Text inside of a frame')],      
                    [sg.CB('Check 1'), sg.CB('Check 2')],
                    [sg.Image(data='', background_color='green', size=(320, 240), key='visual', tooltip ='Imagen Visual')]      
                  ]
  _layout = [
              [
